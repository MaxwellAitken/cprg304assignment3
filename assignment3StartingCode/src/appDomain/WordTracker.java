...
/**
 * WordTracker that tracks words, their occurrences in files, and generates reports.
 */
    ...
    private String formatReportLine(String word, WordMeta meta, String command) {
        ...
        meta.getOccurrences().forEach((file, lines) -> {
            ...
            if (!command.equals("-pf")) {
            	line.append("  on lines: ").append(String.join(", ", lines.stream().map(String::valueOf).collect(Collectors.toList())));
            }
        });
